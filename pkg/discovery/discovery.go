...
// Compile-time assertion that Service implements interfaces.Discovery.
var _ interfaces.Discovery = (*Service)(nil)
...
func (s *Service) TestSourceConnection(ctx context.Context, cfg *interfaces.SourceConfig) (latencyMs int64, err error) {
...
func (s *Service) TestSinkConnection(ctx context.Context, cfg *interfaces.SinkConfig) (latencyMs int64, err error) {
	ctx, cancel := context.WithTimeout(ctx, connectionTimeout)
	defer cancel()
...
func (s *Service) DiscoverSourceTables(ctx context.Context, cfg *interfaces.SourceConfig) ([]interfaces.TableInfo, error) {
	switch cfg.Type {
	case constant.SourceTypePostgres.String():
	...
	default:
		return []interfaces.TableInfo{}, nil
	}
}

// DiscoverSinkTables connects to the sink and returns table/column metadata.
func (s *Service) DiscoverSinkTables(ctx context.Context, cfg *interfaces.SinkConfig) ([]interfaces.TableInfo, error) {
	switch cfg.Type {
	case constant.SinkTypePostgres.String():
...
func testPostgresSourceConnection(ctx context.Context, cfg *interfaces.SourceConfig) error {
...
func testMySQLSourceConnection(ctx context.Context, cfg *interfaces.SourceConfig) error {
...
func testPostgresSinkConnection(ctx context.Context, cfg *interfaces.SinkConfig) error {
	connStr := config.PostgresDSN(cfg.Host, cfg.Port, cfg.Username, cfg.Password, cfg.Database)
...
func testElasticsearchConnection(_ context.Context, cfg *interfaces.SinkConfig) error {
...
func testClickhouseConnection(ctx context.Context, cfg *interfaces.SinkConfig) error {
	addr := cfg.Host
	if cfg.Port > 0 {
...
func discoverPostgresSourceTables(ctx context.Context, cfg *interfaces.SourceConfig) ([]interfaces.TableInfo, error) {
	connStr := config.PostgresDSN(cfg.Host, cfg.Port, cfg.Username, cfg.Password, cfg.Database)
...
func discoverPostgresSinkTables(ctx context.Context, cfg *interfaces.SinkConfig) ([]interfaces.TableInfo, error) {
	connStr := config.PostgresDSN(cfg.Host, cfg.Port, cfg.Username, cfg.Password, cfg.Database)
...
func discoverPostgresTables(ctx context.Context, pool *pgxpool.Pool) ([]interfaces.TableInfo, error) {
	// 1. Discover tables
	...
	// 4. Assemble results
	tables := make([]interfaces.TableInfo, 0, len(tableKeys))
	for _, tk := range tableKeys {
		key := tk.schema + "." + tk.name
		cols := colMap[key]
		columns := make([]interfaces.ColumnInfo, 0, len(cols))
		for _, c := range cols {
...
func discoverMySQLSourceTables(ctx context.Context, cfg *interfaces.SourceConfig) ([]interfaces.TableInfo, error) {
	dsn := config.MySQLDSN(cfg.Host, cfg.Port, cfg.Username, cfg.Password, cfg.Database)
...
func discoverClickhouseSinkTables(ctx context.Context, cfg *interfaces.SinkConfig) ([]interfaces.TableInfo, error) {
	addr := cfg.Host
	if cfg.Port > 0 {
		addr = fmt.Sprintf("%s:%d", cfg.Host, cfg.Port)
	} else if len(cfg.URL) > 0 {
