...
type FlowWorker struct {
	...
	filter      *Filter
	mappings    []interfaces.ColumnMapping
	maxDeliver  int
	log         *slog.Logger
...
func (w *FlowWorker) processBatch(ctx context.Context, msgs []jetstream.Msg) {
	...
	poolEvents := make([]*models.Event, 0, len(msgs))
		...
		if len(w.mappings) > 0 && len(ev.Data) > 0 {
			...
			if err != nil {
				w.log.Warn("column mapping failed, moving event to DLQ",
					"err", err,
					"offset", ev.Offset)
				if dlqErr := w.natsClient.MoveToDLQ(ctx, msg, interfaces.DLQMoveOptions{
...
func (w *FlowWorker) parseEventFromMsg(msg jetstream.Msg) *models.Event {
	ev := pool.GetEvent()
	headers := msg.Headers()
